 ...
 * @LastEditTime: 2022-04-22 12:49:03
 * @Description: 请填写简介
 */
...
func TestUserReturnErrorsInOrder(t *testing.T) {
	...
	testUser := &User{Doer: mockDoer}
	gomock.InOrder(
		mockDoer.EXPECT().DoSomething(1, "first this").Return(dummyError),
		mockDoer.EXPECT().DoSomething(2, "then this").Return(nil),
		mockDoer.EXPECT().DoSomething(3, "then this").Return(dummyError),
		mockDoer.EXPECT().DoSomething(4, "finally this").Return(nil),
	)
	err1 := testUser.Use()
